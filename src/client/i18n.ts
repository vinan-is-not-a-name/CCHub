...
const en: Dict = {
  ...
  'balance.title': 'Relay balance',
  'balance.pill.aria': 'Relay balance — {n} sites',
  'balance.updated': 'Updated {time}',
  'balance.interval': 'auto-refresh {s}s',
  ...
  'launch.loadPreset': 'Load from preset',
  'launch.noPreset': 'Custom',
  ...
  'directory.newFolder.placeholder': 'Folder name',
  'directory.newFolder.aria': 'New folder name',
  'directory.newFolder.create': 'Create',
  'directory.newFolder.cancel': 'Cancel',
  'directory.mkdir.created': 'Folder created',
  'directory.mkdir.error.invalid': 'Invalid folder name',
  'directory.mkdir.error.exists': 'A folder with that name already exists',
  'directory.mkdir.error.denied': 'Permission denied',
  'directory.mkdir.error.failed': 'Could not create folder',
  ...
  'provider.authToken': 'Auth token (env: ANTHROPIC_AUTH_TOKEN — sent as Bearer)',
  'provider.apiKey': 'API key (env: ANTHROPIC_API_KEY — sent as x-api-key)',
  ...
  'provider.clearApiKey': 'Clear saved API key',
  ...
  'settings.tab.sshkey': 'SSH key',
  'settings.sshkey.title': 'cc-remote SSH key',
  'settings.sshkey.subtitle': 'A dedicated key for passwordless XShell login. Separate from your own ~/.ssh keys.',
  'settings.sshkey.publicLabel': 'Public key',
  'settings.sshkey.none': 'No key yet. Click Generate to create one.',
  'settings.sshkey.generate': 'Generate key',
  'settings.sshkey.regenerate': 'Regenerate',
  'settings.sshkey.copy': 'Copy',
  'settings.sshkey.copied': 'Copied.',
  'settings.sshkey.regenWarn': 'Regenerate creates a brand-new key. You must re-import it into XShell and re-install it on every server. Continue?',
  'settings.sshkey.generated': 'Key ready. Import the private key into XShell once, then install the public key on your servers.',
  'settings.sshkey.privateHint': 'Private key file (import this into XShell → Tools → User Key Manager → Import):',
  'settings.sshkey.installHint': 'Install the public key on a server from the Servers tab in the session config dialog.',
  'settings.sshkey.error': 'Something went wrong. Check the cc-remote server logs.',

  'server.sshkey.install': 'Install cc-remote key',
  'server.sshkey.installing': 'Installing…',
  'server.sshkey.checking': 'Checking…',
  'server.sshkey.installed': 'Certificate installed on server',
  'server.sshkey.installedNow': 'Installed. XShell can now log in without a password.',
  'server.sshkey.alreadyInstalled': 'Already installed on this server.',
  'server.sshkey.noKey': 'Generate the cc-remote key first (Settings → SSH key).',
  'server.sshkey.failed': 'Install failed: {error}',
  'server.sshkey.hint': 'Lets XShell open this server without a password prompt.',
...
const zh: Dict = {
  ...
  'balance.title': '中转站余额',
  'balance.pill.aria': '中转站余额 — {n} 个站点',
  'balance.updated': '更新于 {time}',
  'balance.interval': '每 {s} 秒自动刷新',
  'balance.refresh': '刷新',
  'balance.site': '站点',
  'balance.key': '密钥',
  'balance.remain': '余额',
  ...
  'launch.loadPreset': '从预设加载',
  'launch.noPreset': '直接配置',
  'launch.server': '服务器',
  ...
  'directory.newFolder.placeholder': '文件夹名称',
  'directory.newFolder.aria': '新文件夹名称',
  'directory.newFolder.create': '创建',
  'directory.newFolder.cancel': '取消',
  'directory.mkdir.created': '已创建文件夹',
  'directory.mkdir.error.invalid': '文件夹名称无效',
  'directory.mkdir.error.exists': '同名文件夹已存在',
  'directory.mkdir.error.denied': '没有权限',
  'directory.mkdir.error.failed': '无法创建文件夹',
  ...
  'provider.authToken': 'Auth Token（对应环境变量 ANTHROPIC_AUTH_TOKEN，以 Bearer 头发送）',
  'provider.apiKey': 'API Key（对应环境变量 ANTHROPIC_API_KEY，以 x-api-key 头发送）',
  ...
  'provider.clearApiKey': '清除已保存的 API Key',
  'provider.test': '测试连接',
  'provider.delete': '删除',
  ...
  'settings.tab.sshkey': 'SSH 密钥',
  'settings.sshkey.title': 'cc-remote SSH 密钥',
  'settings.sshkey.subtitle': '用于 XShell 免密登录的专用密钥,独立于你自己的 ~/.ssh 密钥。',
  'settings.sshkey.publicLabel': '公钥',
  'settings.sshkey.none': '尚未生成密钥。点击"生成密钥"创建。',
  'settings.sshkey.generate': '生成密钥',
  'settings.sshkey.regenerate': '重新生成',
  'settings.sshkey.copy': '复制',
  'settings.sshkey.copied': '已复制。',
  'settings.sshkey.regenWarn': '重新生成会创建全新密钥。你需要重新导入 XShell 并在每台服务器上重新安装。是否继续?',
  'settings.sshkey.generated': '密钥已就绪。先把私钥导入 XShell 一次,再把公钥安装到各服务器。',
  'settings.sshkey.privateHint': '私钥文件(导入路径:XShell → 工具 → 用户密钥管理者 → 导入):',
  'settings.sshkey.installHint': '在会话配置对话框的"服务器"标签页里把公钥安装到某台服务器。',
  'settings.sshkey.error': '出错了,请查看 cc-remote 服务端日志。',

  'server.sshkey.install': '安装 cc-remote 密钥',
  'server.sshkey.installing': '安装中…',
  'server.sshkey.checking': '检查中…',
  'server.sshkey.installed': '已安装证书到服务器',
  'server.sshkey.installedNow': '已安装。XShell 现在可以免密登录此服务器。',
  'server.sshkey.alreadyInstalled': '此服务器已安装过。',
  'server.sshkey.noKey': '请先生成 cc-remote 密钥(设置 → SSH 密钥)。',
  'server.sshkey.failed': '安装失败:{error}',
  'server.sshkey.hint': '让 XShell 打开此服务器时无需输入密码。',
